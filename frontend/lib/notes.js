...
export function NotesProvider({ children }) {
  ...
  function addNote(ayahKey, noteData) {
    setNotes((prev) => {
      const updated = { ...prev, [ayahKey]: { ...noteData, savedAt: Date.now(), updatedAt: Date.now() } };
      save(updated);
      return updated;
    });
  }
  ...
  function updateNote(ayahKey, text) {
    setNotes((prev) => {
      ...
      const updated = { ...prev, [ayahKey]: { ...prev[ayahKey], text, updatedAt: Date.now() } };
      save(updated);
      return updated;
    });
  }
  ...
  return (
    <NotesContext.Provider value={{ notes, addNote, updateNote, deleteNote, getNote, hasNote, clearAll, count, mounted }}>
      {children}
